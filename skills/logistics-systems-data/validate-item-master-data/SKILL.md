---
name: validate-item-master-data
description: Validate logistics item master data for UOM, pack hierarchy, dimensions, weights, identifiers, controls, statuses, and operational completeness.
license: MIT
---
...
Use this skill to support logistics systems and data analysis for operational systems. The expected output is an item-data validation with source evidence, assumptions, conflicts, controls, and review boundaries.
...
- validate item master fields, SKU setup, UOM, pack hierarchy, dimensions, weight, barcode, GTIN, lot, serial, expiry, hazardous, or temperature flags
- review item setup before WMS, ERP, OMS, slotting, receiving, picking, barcode, EDI, or API work
- find item-data defects that could cause inventory, picking, replenishment, cartonization, receiving, or shipping problems

## Non-Triggers

Do not use this skill when the user primarily needs to:

- approve a master-data change, item launch, regulatory classification, dangerous-goods declaration, or financial setup
- write directly to ERP, WMS, OMS, PIM, MDM, EDI, API, or label systems
- certify barcode, GS1, tax, customs, food, pharma, hazmat, or legal compliance
...
- item identifier, description, status, owner system, facility or network scope, and extraction timestamp
- base UOM, alternate UOMs, pack hierarchy, conversion factors, dimensions, weight, cube, and handling units
- barcode or GTIN fields, lot/serial/expiry controls, catch-weight, hazardous, temperature, and storage attributes when applicable
- processes affected by the item setup, such as receiving, putaway, picking, replenishment, cartonization, shipping, and inventory control
- source records or exports used for validation

## Optional Inputs

Use when available:

- supplier item data, customer item data, label samples, EDI records, API payloads, photographs, and physical measurements
- item velocity, ABC class, storage requirement, cartonization rules, demand profile, and slotting rules
- current errors, exception codes, short picks, receiving discrepancies, and WMS transaction history
...
1. Confirm item scope, owner system, source records, and validation purpose.
2. Check completeness, validity, consistency, uniqueness, timeliness, lineage, and operational fit of item fields.
3. Compare UOM conversions, pack hierarchy, dimensions, weights, identifiers, controls, and statuses across systems.
4. Identify defects, source conflicts, operational risks, and fields requiring owner confirmation.
5. Return an item-master validation report with evidence and change-control boundaries.

## Calculations

Optional checks can calculate cube from dimensions, compare weight and cube by UOM, and verify pack-hierarchy conversions. Use shared unit guidance and block final conclusions when units, conversion factors, or measurement sources conflict.
...
- base UOM and alternate UOM conversion logic are visible
- pack hierarchy aligns with dimensions, weight, and operational process needs
- identifiers are treated as source fields unless official identifier rules are verified
- data defects are separated from recommended master-data changes
- approval and system-write boundaries are explicit
...
- item data validation with scope, source records, and source-system lineage
...
Use this skill when a SKU has EA and CS units, a case pack of 12 in ERP, a WMS pack of 10, missing dimensions, a GTIN field, lot control enabled, and repeated pick exceptions.
...
- pack hierarchy mismatch
- missing dimensions and weight
- lot or serial control conflict
- master-data approval boundary
