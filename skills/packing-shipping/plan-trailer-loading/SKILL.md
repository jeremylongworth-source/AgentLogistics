---
name: plan-trailer-loading
description: Plan trailer loading from shipments, pallets, weights, stops, sequence, equipment, and load-safety boundaries.
license: MIT
---
...
Use this skill to prepare a trailer loading plan for pallets, cartons, stops, routes, or outbound shipments. The expected output is a trailer loading plan with sequence, cube and weight checks, constraints, and review boundaries.
...
- plan trailer loading, load sequence, truck loading, outbound load plan, or stop sequence loading
- compare shipments against trailer space, weight, route, stop, or service constraints
- prepare a loading plan before carrier handoff
...
- hazard, temperature, blocking and bracing, axle, floor-load, load-securement, carrier, and jurisdiction evidence supplied for review
- pallet stackability, fragility, orientation, overhang, and segregation rules
...
1. Confirm trailer scope, units, and loading objective.
2. Check shipment eligibility, dimensions, weights, stop sequence, and special handling constraints.
3. Estimate cube, floor position, and weight use from supplied data.
4. Sequence load by stop, stability, accessibility, and operational constraints.
5. Return a loading plan with unresolved safety and compliance review points.

## Calculations

Use `load cube = sum(load length * width * height)` and `cube utilization % = load cube / usable trailer cube * 100` when dimensions are supplied. Use `weight utilization % = load weight / stated trailer capacity * 100`. Use `floor positions required` only when pallet footprint and trailer floor dimensions are supplied. Do not approve axle, securement, floor-load, or legal weight compliance.
...
- a trailer loading plan with sequence, cube and weight checks, constraints, and review boundaries
