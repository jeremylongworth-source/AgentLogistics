---
name: investigate-picking-error
description: Investigate picking errors by tracing order, pick record, SKU, location, scanner, pack, ship, and customer evidence.
license: MIT
---
...
Use this skill to investigate a specific picking error using source records and event chronology. The expected output is a picking error investigation with evidence table, chronology, candidate causes, and corrective handoffs.
...
- investigate picking error, wrong item, short pick, overpick, mispick, or missing item
- trace order, SKU, location, scanner event, pick record, pack verification, and shipment evidence
- prepare corrective action evidence before inventory, customer, or process changes
...
- photos, customer claim, return evidence, slotting context, inventory count, replenishment status, and pack audit
- picker, shift, device, tote, carton, and staging lane identifiers
...
1. Freeze the order line, SKU, location, and event time window.
2. Build a source-by-source evidence table for order, pick, scan, pack, ship, and customer records.
3. Order events into a chronology and identify the first conflicting handoff.
4. Rank candidate causes by cited evidence rather than confidence.
5. Return missing evidence, containment checks, and corrective-action handoffs.

## Calculations

No fixed calculation required. Optional quantity variance uses `pick variance = picked quantity - ordered quantity` and `ship variance = shipped quantity - ordered quantity` when units match. Do not infer theft, misconduct, or operator fault from incomplete records.
...
- a picking error investigation with evidence table, chronology, candidate causes, and corrective handoffs
