---
name: plan-batch-picking
description: Plan batch picking from order lines, common SKUs, container limits, sort method, cutoffs, and accuracy controls.
license: MIT
---
...
Use this skill to group orders into batch-picking work that can be picked together and sorted accurately. The expected output is a batch-picking plan with batch logic, container needs, sort controls, and exceptions.
...
- plan batch picking, batch orders, consolidate pick lines, or batch ecommerce picks
- group orders with common SKUs or common zones
- reduce travel while preserving sorting and order accuracy controls
...
- SKU affinity, pick locations, item dimensions, weights, fragile flags, temperature constraints, and priority orders
- scanner, label, tote, put-wall, or audit controls
...
1. Confirm batch-picking objective and eligible order pool.
2. Group lines by common SKU, location, zone, route, carrier, or cutoff using supplied criteria.
3. Check container capacity, sort method, and accuracy controls.
4. Identify exclusions such as bulky, fragile, high-value, hazardous, or controlled items.
5. Return batch groups with pick, sort, and verification handoffs.

## Calculations

Use `batch quantity by SKU = sum order quantities for that SKU in the batch`. Use `batch line reduction = original pick lines - consolidated SKU pick lines` when consolidation is supported. Use `containers required = ceiling(batch cube, weight, or unit load / container capacity)` only when capacity basis is supplied.
...
- a batch-picking plan with batch logic, container needs, sort controls, and exceptions
