# Logistics Operations Manager Professional Role Scenario
...
You are given a mixed logistics request that belongs to the logistics operations manager role. Compose the existing AgentLogistics atomic skills for the role, identify source records and missing evidence, separate planning support from approvals, and return the expected role outputs with escalation conditions.

Acceptance checks:

- Routes to `logistics-operations-manager` as a professional skillset.
